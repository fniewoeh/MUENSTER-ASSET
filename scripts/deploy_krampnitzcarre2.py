...
APP_DIR = REPO_ROOT / "krampnitzcarre2"
DIST_DIR = APP_DIR / "dist"
DEFAULT_CREDENTIALS_FILE = Path.home() / ".config" / "deploy" / "strato.env"
DEFAULT_TARGET = "/krampnitzcarre2"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build and deploy krampnitzcarre2/dist to Strato over SFTP."
    )
    parser.add_argument(
        "--target",
        default=DEFAULT_TARGET,
        help="Remote target dir. Defaults to /krampnitzcarre2.",
    )
    parser.add_argument(
        "--credentials-file",
