...
const CONFIG_STORAGE_KEY = 'tq-home-runtime-config'
...
const heroSlides: HeroSlide[] = [
  {
    image: '/project/hero-tq-01.png',
    alt: 'TQ Home Einleitung aus dem Prospekt',
    caption: 'TQ Home am Fleetpark',
  },
  {
    image: '/project/hero-tq-02.png',
    alt: 'TQ Home Lage im Tabakquartier',
    caption: 'Lage in Bremen',
  },
  {
    image: '/project/hero-tq-03.png',
    alt: 'TQ Home Architektur',
    caption: 'Architektur und Quartier',
  },
  {
    image: '/project/hero-tq-04.png',
    alt: 'TQ Home Ausstattung',
    caption: 'Ausstattung im Überblick',
  },
  {
              ...
              <div class="config-grid config-grid-compact">
                <label class="config-field">
                  ...
                  <span class="config-field-hint">
                    Wird als Dateiname und URL-Parameter genutzt, z. B. tq-home-standard-01.
                  </span>
                  <div class="config-input-wrap">
                    <input
                      ...
                      spellcheck="false"
                      autocomplete="off"
                      placeholder="tq-home-standard-01"
                    />
                  </div>
                </label>
                <label class="config-field">
  ...
  <main class="page">
    <section class="panel hero${appMode === 'customer' ? ' hero-customer' : ''}">
      ...
      <div class="hero-content">
        <p class="eyebrow">TQ Home im Tabakquartier</p>
    ...
    <section class="panel facts-panel">
      <h2>Ein paar kurze Fakten zum Tabakquartier.</h2>
      <p class="lead">
        Die wichtigsten Standort- und Quartiersdaten direkt im kompakten Überblick.
      </p>
      <div class="facts-grid">
        <article class="fact-card">
          <p class="fact-number">20.000 m²</p>
          <p class="fact-title">Fleetpark direkt vor der Tür</p>
          <div class="fact-bar">
            <div class="fact-fill fact-fill-demand" style="width: 92%"></div>
          </div>
          <p class="fact-copy">TQ Home liegt unmittelbar am großen Quartierspark im Tabakquartier.</p>
        </article>

        <article class="fact-card">
          <p class="fact-number">1500</p>
          <p class="fact-title">Wohnungen im Quartier</p>
          <div class="compare-row">
            <span>Neuer urbaner Wohnraum</span>
            <span>direkt im Tabakquartier</span>
          </div>
          <div class="fact-bar">
        ...
        <article class="fact-card">
          <p class="fact-number">4000</p>
          <p class="fact-title">Arbeitsplätze im Quartier</p>
          <div class="compare-row">
            <span>Buero, Handel und Dienstleistungen</span>
            <span>im direkten Umfeld</span>
          </div>
          <div class="dual-bars">
            <div class="dual-bar">
        ...
        <article class="fact-card">
          <p class="fact-number">1,9 km</p>
          <p class="fact-title">Bis in die Bremer Innenstadt</p>
          <p class="fact-copy">Weitere Distanzen laut Prospekt: Hbf ca. 2,7 km und Flughafen ca. 2,4 km.</p>
          <p class="fact-copy">Das Quartier kombiniert Stadtlage, Grünraum und kurze Wege.</p>
        </article>
      </div>
    </section>
...
function calculateProjection(
  ...
  let migratedKfwBankTargetDebtService = 0
  let migratedKfwRefinanceTargetDebtService = 0
  ...
  for (let year = 1; year <= assumptions.years; year += 1) {
    ...
    if (year === kfwFixedRateYears + 1) {
      migratedKfwBankTargetDebtService = remainingKfwDebt * (assumptions.bankInterestRate + assumptions.bankRepaymentRate)
    }

    if (year === bankZinsbindungEndProjectionYear + 1) {
      refinanceDebtBase += remainingKfwDebt + remainingBankDebt
      migratedKfwRefinanceTargetDebtService =
        remainingKfwDebt * (refinanceInterestRate + refinanceRepaymentRate)
      bankRefinanceTargetDebtService =
        remainingBankDebt * (refinanceInterestRate + refinanceRepaymentRate)
    }

    if (year <= kfwFixedRateYears) {
    ...
    } else if (year <= bankZinsbindungEndProjectionYear) {
      const migratedKfwInterest = remainingKfwDebt * assumptions.bankInterestRate
      const migratedKfwPrincipal = Math.min(
        Math.max(migratedKfwBankTargetDebtService - migratedKfwInterest, 0),
        remainingKfwDebt,
      )
      bankInterest += migratedKfwInterest
      bankPrincipal += migratedKfwPrincipal
      remainingKfwDebt -= migratedKfwPrincipal
    } else {
      const currentRefinanceInterest = remainingKfwDebt * refinanceInterestRate
      const currentRefinancePrincipal = Math.min(
        Math.max(migratedKfwRefinanceTargetDebtService - currentRefinanceInterest, 0),
        remainingKfwDebt,
      )
      refinanceInterest += currentRefinanceInterest
      refinancePrincipal += currentRefinancePrincipal
      remainingKfwDebt -= currentRefinancePrincipal
    }
    ...
    if (year <= bankZinsbindungEndProjectionYear) {
      ...
      remainingBankDebt -= normalBankPrincipal * rentShare
    } else {
      const currentRefinanceInterest = remainingBankDebt * refinanceInterestRate
      const currentRefinancePrincipal = Math.min(
        Math.max(bankRefinanceTargetDebtService - currentRefinanceInterest, 0),
        remainingBankDebt,
      )
      refinanceInterest += currentRefinanceInterest
      refinancePrincipal += currentRefinancePrincipal
      remainingBankDebt -= currentRefinancePrincipal
    }
    ...
    const yearlySpecialAfa7bAmount =
      assumptions.specialAfa7bPerSqm > 0 && assumptions.specialAfa7bYears > 0
        ? specialAfa7bBase * getSpecialAfa7bFactorForProjectionYear(year)
        : 0
    const yearlyTotalInterest = kfwInterest + bankInterest + refinanceInterest
    const vuvEinkuenfte =
      yearlyGrossRent -
      yearlyDeductibleOperatingCosts -
      yearlyTotalInterest -
      yearlyAfaAmount -
      yearlySpecialAfa7bAmount
    ...
    if (yearlyAfaAmount > 0 || yearlySpecialAfa7bAmount > 0) {
...
function calculateKfwRepaymentRate(sourceAssumptions: Assumptions): number {
  ...
  const interestRate = Math.max(sourceAssumptions.kfwInterestRate, 0)

  if (interestRate === 0) {
    return 1 / repaymentYears
  }

  const annuityFactor = interestRate / (1 - Math.pow(1 + interestRate, -repaymentYears))
  return Math.max(annuityFactor - interestRate, 0)
}

function estimateKfwLoanTermYears(
  interestRate: number,
  repaymentRate: number,
  graceYears: number,
): number {
  const safeGraceYears = Math.max(Math.round(graceYears), 0)
  const safeRepaymentRate = Math.max(repaymentRate, 0)
  const safeInterestRate = Math.max(interestRate, 0)

  if (safeRepaymentRate === 0) {
    return safeGraceYears + 30
  }

  let repaymentYears = 30
  if (safeInterestRate === 0) {
    repaymentYears = Math.max(1, Math.round(1 / safeRepaymentRate))
  } else {
    const annuityFactor = safeInterestRate + safeRepaymentRate
    if (annuityFactor > safeInterestRate) {
      const ratio = 1 - safeInterestRate / annuityFactor
      if (ratio > 0 && ratio < 1) {
        repaymentYears = Math.max(1, Math.round(-Math.log(ratio) / Math.log(1 + safeInterestRate)))
      }
    }
  }

  return safeGraceYears + repaymentYears
}
  ...
  return [
    {
      ...
      fields: [
        ...
        {
          type: 'number',
          id: 'config-kfw-loan-term-years',
          label: 'KfW-Laufzeit',
          hint: 'Gesamtlaufzeit des KfW-Darlehens inklusive tilgungsfreier Jahre.',
          mode: 'number',
          min: 1,
          max: 40,
          step: 1,
          get: (value) => value.assumptions.kfwLoanTermYears,
          set: (value, next) => {
            value.assumptions.kfwLoanTermYears = next
          },
        },
        {
          type: 'number',
          ...
          label: 'KfW-Tilgung',
          hint: 'Automatisch aus Zins, Laufzeit und tilgungsfreien Jahren berechnet.',
          mode: 'percent',
          min: 0,
          max: 100,
          step: 0.01,
    ...
    {
      title: 'Wohnung A',
      copy: 'TQ Home WE 13 aus dem Prospekt.',
      open: false,
      fields: [
        {
          type: 'number',
    ...
    },
    {
      ...
      fields: [
        ...
        },
        {
          ...
          hint: 'Zusätzliche Sonderabschreibung in EUR pro m² für die ersten 4 Jahre.',
          mode: 'currency',
          min: 0,
          step: 10,
    ...
    <label class="config-field" for="${field.id}">
      ...
      <span class="config-input-wrap">
        <input
          ...
          step="${field.step}"
          inputmode="decimal"
          value="${formatConfigInputValue(field.get(sourceConfig), field.mode)}"${minAttribute}${maxAttribute}${readonlyAttribute}
        />
...
function normalizeDerivedConfigValues(nextConfig: CalculationConfig): void {
  ...
  nextConfig.assumptions.kfwLoanTermYears = Math.max(
    Math.round(nextConfig.assumptions.kfwGraceYears) + 1,
    Math.round(nextConfig.assumptions.kfwLoanTermYears),
  )
  nextConfig.assumptions.kfwZinsbindungJahre = 10
  nextConfig.assumptions.bankZinsbindungJahre = Math.max(
    1,
    Math.round(nextConfig.assumptions.bankZinsbindungJahre),
  )
  nextConfig.assumptions.kfwRepaymentRate = calculateKfwRepaymentRate(nextConfig.assumptions)
...
function validateConfig(candidate: unknown): CalculationConfig {
  ...
  const legacyZinsbindungJahre = asOptionalNumber(
    assumptionsCandidate.zinsbindungJahre,
    10,
    'assumptions.zinsbindungJahre',
  )
  const legacyKfwRepaymentRate = asOptionalNumber(
    assumptionsCandidate.kfwRepaymentRate,
    0.024,
    'assumptions.kfwRepaymentRate',
  )
  const fallbackKfwLoanTermYears = estimateKfwLoanTermYears(
    asNumber(assumptionsCandidate.kfwInterestRate, 'assumptions.kfwInterestRate'),
    legacyKfwRepaymentRate,
    asNumber(assumptionsCandidate.kfwGraceYears, 'assumptions.kfwGraceYears'),
  )
  const assumptions = {
    ...
    kfwLoanTermYears: asOptionalNumber(
      assumptionsCandidate.kfwLoanTermYears,
      fallbackKfwLoanTermYears,
      'assumptions.kfwLoanTermYears',
    ),
    kfwZinsbindungJahre: asOptionalNumber(
      assumptionsCandidate.kfwZinsbindungJahre,
      10,
      'assumptions.kfwZinsbindungJahre',
    ),
    bankZinsbindungJahre: asOptionalNumber(
      assumptionsCandidate.bankZinsbindungJahre,
      legacyZinsbindungJahre,
    ...
    specialAfa7bPerSqm: asOptionalNumber(
      assumptionsCandidate.specialAfa7bPerSqm,
      0,
      'assumptions.specialAfa7bPerSqm',
    ),
    specialAfa7bYears: asOptionalNumber(
      assumptionsCandidate.specialAfa7bYears,
      0,
  ...
  assumptions.kfwLoanTermYears = Math.max(Math.round(assumptions.kfwGraceYears) + 1, Math.round(assumptions.kfwLoanTermYears))
  assumptions.kfwZinsbindungJahre = 10
  assumptions.kfwRepaymentRate = calculateKfwRepaymentRate(assumptions)
