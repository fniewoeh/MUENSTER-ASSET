...
type Assumptions = {
  ...
  landShare: number
  oldBuildingShare: number
  constructionShare: number
  monumentShare: number
  ordinaryAfaRate: number
...
const CONFIG_STORAGE_KEY = 'krampnitzcarre2-runtime-config'
const PREVIEW_PRESET_STORAGE_KEY = 'krampnitzcarre2-preview-preset'
const PREVIEW_PRESET_QUERY_KEY = 'previewPreset'
const DEFAULT_PRESET_ID = 'krampnitzcarre2-default'
const apartmentSizeBounds = { min: 24.7, max: 125.6, step: 0.1 }
const baseApartmentSize = 24.7
const baseApartmentPrice = 230900
const fixedRentPerSqm = 18
const fixedMonthlyManagement = 70
const monthlyMaintenancePerSqm = 1
const fixedMonumentShare = 0.68
...
const MAPS_URL = 'https://www.mlp-anlageimmobilien.de/krampnitzcarre2'
...
const CUSTOMER_SCENARIO_DATA_ROOT = '/krampnitzcarre2-data/customer-scenarios'
...
const heroSlides: HeroSlide[] = [
  ...Array.from({ length: 16 }, (_, index) => {
    const page = 27 + index
    return {
      image: `/project/krampnitzcarre2-page-${page}.jpg`,
      alt: `Krampnitzcarre 2 Projektbild Seite ${page}`,
      caption: `Projektbild Seite ${page}`,
    }
  }),
]
...
app.innerHTML = `
              ...
              <div class="config-grid config-grid-compact">
                <label class="config-field">
                  ...
                  <span class="config-field-hint">
                    Wird als Dateiname und URL-Parameter genutzt, z. B. krampnitzcarre2-kunde-01.
                  </span>
                  <div class="config-input-wrap">
                    <input
                      ...
                      spellcheck="false"
                      autocomplete="off"
                      placeholder="krampnitzcarre2-kunde-01"
                    />
                  </div>
                </label>
                <label class="config-field">
                  ...
                  <div class="config-input-wrap">
                    <input
                      ...
                      type="text"
                      autocomplete="off"
                      placeholder="Krampnitzcarre 2 Kunde 01"
                    />
                  </div>
                </label>
              </div>
  ...
  <main class="page">
    <section class="panel hero${appMode === 'customer' ? ' hero-customer' : ''}">
      ...
      <div class="hero-content">
        <p class="eyebrow">Krampnitzcarre 2 Potsdam</p>
        ...
        <p class="lead">
          Nachfolgeprojekt in Potsdam: Stellen Sie die Wohnungsgröße ein und erhalten Sie sofort eine
          transparente ${projectionYears}-Jahres-Prognose für Vermögen, Liquidität und Steuereffekt.
        </p>
        ${appMode === 'customer'
          ? `
    ...
    <section class="journey-strip" aria-label="So funktioniert der Immobilien-Check">
      <article class="journey-step">
        ...
        <div>
          <p class="journey-step-title">Wohnungsgröße einstellen</p>
          <p class="journey-step-copy">Die Fläche per Schieberegler wählen; Kaufpreis und KfW-Anteil folgen automatisch.</p>
        </div>
      </article>
      <article class="journey-step">
    ...
    <section class="workspace">
      <section class="panel choose-panel">
        <h2>1. Wählen Sie Ihre Wohnungsgröße</h2>
        <label class="field apartment-size-field" for="apartment-size">
          <span>Wohnungsgröße</span>
          <input
            id="apartment-size"
            type="range"
            min="${apartmentSizeBounds.min}"
            max="${apartmentSizeBounds.max}"
            step="${apartmentSizeBounds.step}"
          />
          <strong id="out-apartment-size" class="slider-value">-</strong>
        </label>
    ...
    <section class="panel facts-panel">
      <h2>Ein paar schnelle Fakten zum Krampnitzcarre 2.</h2>
      <p class="lead">
        Das Nachfolgeprojekt nutzt einen variablen Wohnungsgrößen-Rechner. Fotos und Varianten sind bewusst entfernt,
        bis die finalen Projektunterlagen vorliegen.
      </p>
      <div class="facts-grid">
        <article class="fact-card">
          <p class="fact-number">24,70-125,60 m²</p>
          <p class="fact-title">wählbare Wohnungsgröße</p>
          <div class="fact-bar">
            <div class="fact-fill fact-fill-demand" style="width: 8%"></div>
          </div>
          <p class="fact-copy">Die Fläche wird direkt im Rechner eingestellt.</p>
        </article>

        <article class="fact-card">
          <p class="fact-number">230.900 EUR</p>
          <p class="fact-title">Einstiegspreis</p>
          <div class="compare-row">
            <span>kleinste Einheit: 24,70 m²</span>
            <span>Preis wird linear hochgerechnet</span>
          </div>
          <div class="fact-bar">
            <div class="fact-fill fact-fill-single" style="width: 53.4%"></div>
          </div>
        </article>

        <article class="fact-card">
          <p class="fact-number">max. 150.000 EUR</p>
          <p class="fact-title">KfW-Anteil</p>
          <div class="compare-row">
            <span>zusätzlich begrenzt auf 60 % des Kaufpreises</span>
            <span>Tilgungszuschuss: 25 %</span>
          </div>
          <div class="dual-bars">
            <div class="dual-bar">
        ...
        <article class="fact-card">
          <p class="fact-number">KfW + Denkmalmodell</p>
          <p class="fact-title">Förder- und Sanierungsrahmen</p>
          <p class="fact-copy">Im Exposé als Baudenkmal mit KfW-EH70-EE-WPB ausgewiesen.</p>
          <p class="fact-copy">Für Wohnungen rechnen wir mit 18 EUR/m², für Teileigentum im Erdgeschoss mit 12 EUR/m².</p>
        </article>
      </div>
    </section>
  ...
  <div id="floorplan-modal" class="dialog-modal floorplan-modal" aria-hidden="true">
    <div class="dialog-modal-backdrop" data-floorplan-close="true"></div>
    <section
      class="dialog-modal-panel floorplan-modal-panel"
      role="dialog"
      aria-modal="true"
    ...
    >
      <header class="dialog-modal-head">
        <div>
          <p class="eyebrow">Grundrissansicht</p>
          <h3 id="floorplan-modal-title">Grundriss</h3>
        </div>
        <button
          id="floorplan-modal-close"
          type="button"
          class="liquidity-view-nav"
          aria-label="Grundrissansicht schließen"
        >
          ×
        </button>
      </header>
      <div class="dialog-modal-content floorplan-modal-content">
        <img id="floorplan-modal-image" class="floorplan-modal-image" src="" alt="" />
        <div id="floorplan-modal-controls" class="floorplan-modal-controls" hidden>
          <button id="floorplan-modal-prev" type="button" class="floorplan-gallery-nav" aria-label="Vorherigen Grundriss anzeigen">
            ‹
          </button>
          <div id="floorplan-modal-dots" class="floorplan-modal-dots" aria-label="Grundrissauswahl"></div>
          <button id="floorplan-modal-next" type="button" class="floorplan-gallery-nav" aria-label="Nächsten Grundriss anzeigen">
            ›
          </button>
        </div>
      </div>
...
const apartmentSizeInput = getElementById<HTMLInputElement>('apartment-size')
...
const floorplanModal = getElementById<HTMLDivElement>('floorplan-modal')
const floorplanModalCloseButton = getElementById<HTMLButtonElement>('floorplan-modal-close')
const floorplanModalTitle = getElementById<HTMLElement>('floorplan-modal-title')
const floorplanModalImage = getElementById<HTMLImageElement>('floorplan-modal-image')
const floorplanModalControls = getElementById<HTMLDivElement>('floorplan-modal-controls')
const floorplanModalPrevButton = getElementById<HTMLButtonElement>('floorplan-modal-prev')
const floorplanModalNextButton = getElementById<HTMLButtonElement>('floorplan-modal-next')
const floorplanModalDots = getElementById<HTMLDivElement>('floorplan-modal-dots')

let selectedApartmentId: ApartmentId = scenarioDefaults.apartmentId
let selectedApartmentSize = getApartment(selectedApartmentId).size
...
let isFloorplanModalOpen = false
let floorplanModalApartmentId: ApartmentId | null = null
let floorplanGalleryIndex = 0
let floorplanGalleryIntervalId: number | null = null
const apartmentCardGalleryIntervalIds: number[] = []
let pendingPresetOpenAction: (() => Promise<void>) | null = null

hydrateStateFromUrl()
updateVariableApartment(selectedApartmentSize)
renderApartmentCards()
renderTaxTableSelection()
writeApartmentSizeInputValue(selectedApartmentSize)
...
apartmentSizeInput.addEventListener('input', () => {
  selectedApartmentSize = clamp(
    parseNumber(apartmentSizeInput.value, selectedApartmentSize),
    apartmentSizeBounds.min,
    apartmentSizeBounds.max,
  )
  updateVariableApartment(selectedApartmentSize)
  writeApartmentSizeInputValue(selectedApartmentSize)
  investedEquity = getDefaultEquityForApartment(selectedApartmentId)
  updateEquityInputConstraints(selectedApartmentId)
  writeEquityInputValue(investedEquity)
  renderApartmentCards()
  renderConfigEditorSummary()
  refreshEditorDirtyState()
...
floorplanModalCloseButton.addEventListener('click', () => {
  closeFloorplanModal()
})

floorplanModalPrevButton.addEventListener('click', () => {
  shiftFloorplanGallery(-1)
  restartFloorplanAutoplay()
})

floorplanModalNextButton.addEventListener('click', () => {
  shiftFloorplanGallery(1)
  restartFloorplanAutoplay()
})
...
floorplanModal.addEventListener('click', (event) => {
  ...
  const closeTrigger = target.closest<HTMLElement>('[data-floorplan-close="true"]')
  if (!closeTrigger) {
    return
  }
  closeFloorplanModal()
...
function renderApartmentCards(): void {
  stopApartmentCardGalleries()
  const apartment = getApartment(selectedApartmentId)
  const maxKfwLoan = getMaxKfwLoan(apartment, apartment.purchasePrice, assumptions.kfwLoanAmount)
  apartmentContainer.innerHTML = `
    <article class="apartment-card apartment-card-active apartment-card-static">
      <button
        class="apartment-image-wrap"
        type="button"
        aria-label="Beispielgrundriss größer ansehen"
        data-floorplan-trigger="true"
      >
        <img src="${resolvePublicAssetPath(apartment.image)}" alt="Beispielgrundriss ${apartment.label}" />
        <span class="apartment-image-zoom">Beispielgrundriss ansehen</span>
      </button>
      ...
      <div class="apartment-summary-grid">
        <span>Wohnfläche <strong>${formatArea(apartment.size)} m²</strong></span>
        <span>KfW-Anteil <strong>${formatCurrency(maxKfwLoan)}</strong></span>
        <span>Tilgungszuschuss <strong>${formatCurrency(maxKfwLoan * assumptions.kfwGrantRate)}</strong></span>
      </div>
    </article>
  `

  apartmentContainer.querySelector<HTMLButtonElement>('[data-floorplan-trigger="true"]')?.addEventListener('click', () => {
    openFloorplanModal(apartment, 0)
  })
}

function getApartmentGalleryImages(apartment: ApartmentOption): string[] {
  return apartment.galleryImages.length > 0 ? apartment.galleryImages : apartment.image ? [apartment.image] : []
}

function stopApartmentCardGalleries(): void {
  while (apartmentCardGalleryIntervalIds.length > 0) {
    const intervalId = apartmentCardGalleryIntervalIds.pop()
...
function renderFloorplanModalImage(): void {
  if (!floorplanModalApartmentId) {
    return
  }
  const apartment = getApartment(floorplanModalApartmentId)
  const galleryImages = getApartmentGalleryImages(apartment)
  if (galleryImages.length === 0) {
    return
  }
  const normalizedIndex = ((floorplanGalleryIndex % galleryImages.length) + galleryImages.length) % galleryImages.length
  const activeImage = galleryImages[normalizedIndex]

  floorplanGalleryIndex = normalizedIndex
  floorplanModalTitle.textContent = apartment.label
  floorplanModalImage.src = resolvePublicAssetPath(activeImage)
  floorplanModalImage.alt = `Grundriss ${apartment.label}`
  floorplanModalControls.hidden = galleryImages.length < 2
  floorplanModalPrevButton.hidden = galleryImages.length < 2
  floorplanModalNextButton.hidden = galleryImages.length < 2
  floorplanModalDots.innerHTML = galleryImages
    .map(
      (_, index) => `
        <button
          type="button"
          class="floorplan-dot${index === normalizedIndex ? ' floorplan-dot-active' : ''}"
          data-floorplan-dot="${index}"
          aria-label="Grundriss ${index + 1} von ${galleryImages.length} anzeigen"
  ...
  floorplanModalDots.querySelectorAll<HTMLButtonElement>('[data-floorplan-dot]').forEach((button) => {
    button.addEventListener('click', () => {
      const index = Number.parseInt(button.dataset.floorplanDot ?? '0', 10)
      floorplanGalleryIndex = Number.isFinite(index) ? index : 0
      renderFloorplanModalImage()
      restartFloorplanAutoplay()
    })
  })
}

function shiftFloorplanGallery(step: number): void {
  if (!floorplanModalApartmentId) {
    return
  }
  const apartment = getApartment(floorplanModalApartmentId)
  ...
  floorplanGalleryIndex = (floorplanGalleryIndex + step + galleryImages.length) % galleryImages.length
  renderFloorplanModalImage()
}

function startFloorplanAutoplay(): void {
  if (!floorplanModalApartmentId || floorplanGalleryIntervalId !== null) {
    return
  }
  const apartment = getApartment(floorplanModalApartmentId)
  if (getApartmentGalleryImages(apartment).length < 2) {
    return
  }
  floorplanGalleryIntervalId = window.setInterval(() => {
    shiftFloorplanGallery(1)
  }, 3600)
}

function stopFloorplanAutoplay(): void {
  if (floorplanGalleryIntervalId === null) {
    return
  }
  window.clearInterval(floorplanGalleryIntervalId)
  floorplanGalleryIntervalId = null
}

function restartFloorplanAutoplay(): void {
  stopFloorplanAutoplay()
  startFloorplanAutoplay()
}

function openFloorplanModal(apartment: ApartmentOption, startIndex = 0): void {
  if (getApartmentGalleryImages(apartment).length === 0) {
    return
  }
  floorplanModalApartmentId = apartment.id
  floorplanGalleryIndex = startIndex
  renderFloorplanModalImage()
  floorplanModalTitle.textContent = apartment.label
  floorplanModal.classList.add('dialog-modal-open')
  floorplanModal.setAttribute('aria-hidden', 'false')
  isFloorplanModalOpen = true
  syncBodyModalState()
  startFloorplanAutoplay()
  window.setTimeout(() => floorplanModalCloseButton.focus(), 40)
}

function closeFloorplanModal(): void {
  if (!isFloorplanModalOpen) {
    return
  }
  stopFloorplanAutoplay()
  floorplanModal.classList.remove('dialog-modal-open')
  floorplanModal.setAttribute('aria-hidden', 'true')
  floorplanModalImage.src = ''
  floorplanModalImage.alt = ''
  floorplanModalDots.innerHTML = ''
  floorplanModalApartmentId = null
  floorplanGalleryIndex = 0
...
function renderProjection(): void {
  ...
  setText('out-apartment-size', `${formatArea(apartment.size)} m²`)
...
function calculateProjection(
  ...
  const annualBaseRent =
    (apartment.size * apartment.rentPerSqm + apartment.accessoryArea * apartment.accessoryRentPerSqm) * 12
  const annualManagementCostsFull = apartment.monthlyManagement * 12
  ...
  const kfwLoan = Math.min(debtNeeded, getMaxKfwLoan(apartment, totalInvestment, assumptions.kfwLoanAmount))
  const bankLoan = Math.max(debtNeeded - kfwLoan, 0)
  const kfwGrant = kfwLoan > 0
    ? Math.min(
        assumptions.kfwGrantRate > 0 ? kfwLoan * assumptions.kfwGrantRate : assumptions.kfwGrantAmount,
        kfwLoan,
      )
    : 0
  ...
  for (let year = 1; year <= assumptions.years; year += 1) {
    ...
    const ordinaryBuildingShare = assumptions.oldBuildingShare + assumptions.constructionShare
    const yearlyBuildingAfaAmount =
      apartment.purchasePrice *
      ordinaryBuildingShare *
      assumptions.ordinaryAfaRate *
      getAfaStartShareForProjectionYear(year)
    const yearlyTotalInterest = kfwInterest + bankInterest + refinanceInterest
    const vuvEinkuenfte = yearlyNetBeforeDebt - yearlyTotalInterest - yearlyAfaAmount - yearlyBuildingAfaAmount
...
function getMaxKfwLoan(apartment: ApartmentOption, totalInvestment: number, defaultKfwLoanAmount: number): number {
  return Math.min(defaultKfwLoanAmount, apartment.purchasePrice * 0.6, totalInvestment)
}
...
function syncBodyModalState(): void {
  document.body.classList.toggle(
    'body-modal-open',
    isLiquidityModalOpen || isCustomerLinkModalOpen || isPresetOpenModalOpen || isConfigEditorOpen || isFloorplanModalOpen,
  )
}
...
function hydrateStateFromUrl(): void {
  ...
  if (size) {
    selectedApartmentSize = clamp(
      parseNumber(size, selectedApartmentSize),
      apartmentSizeBounds.min,
      apartmentSizeBounds.max,
    )
    updateVariableApartment(selectedApartmentSize)
...
function writeApartmentSizeInputValue(value: number): void {
  apartmentSizeInput.value = String(clamp(value, apartmentSizeBounds.min, apartmentSizeBounds.max))
  setText('out-apartment-size', `${formatArea(value)} m²`)
}
...
function updateVariableApartment(size: number): void {
  const apartment = getApartment(selectedApartmentId)
  const normalizedSize = clamp(size, apartmentSizeBounds.min, apartmentSizeBounds.max)
  const purchasePrice = Math.round((baseApartmentPrice / baseApartmentSize) * normalizedSize)
  apartment.size = Number(normalizedSize.toFixed(1))
  apartment.purchasePrice = purchasePrice
  apartment.rooms = normalizedSize < 45 ? 1 : normalizedSize < 85 ? 2 : 3
  apartment.rentPerSqm = fixedRentPerSqm
  apartment.monthlyManagement = fixedMonthlyManagement
  apartment.monthlyMaintenance = Number((apartment.size * monthlyMaintenancePerSqm).toFixed(2))
  apartment.accessoryLabel = ''
  apartment.accessoryArea = 0
  apartment.accessoryRentPerSqm = 0
  apartment.subtitle = buildApartmentSubtitle(apartment)
  assumptions.rentPerSqm = fixedRentPerSqm
  assumptions.monthlyManagementFlat = fixedMonthlyManagement
  assumptions.monthlyMaintenanceFlat = apartment.monthlyMaintenance
  assumptions.monumentShare = fixedMonumentShare
}
...
function getAfaFactorForProjectionYear(year: number): number {
  ...
  const startYearShare = getAfaStartShareForProjectionYear(year)
  return match.rate * startYearShare
}

function getAfaStartShareForProjectionYear(year: number): number {
  const calendarYear = getCalendarYearForProjectionYear(year)
  if (calendarYear < assumptions.afaStartYear) {
    return 0
  }
  ...
  return [
    {
      ...
      fields: [
        ...
        {
          type: 'number',
          id: 'config-kfw-grant-rate',
          label: 'KfW-Tilgungszuschuss',
          hint: 'Förderquote auf das KfW-Darlehen, z. B. 25 %.',
          mode: 'percent',
          min: 0,
          max: 100,
          step: 1,
          get: (value) => value.assumptions.kfwGrantRate,
          set: (value, next) => {
            value.assumptions.kfwGrantRate = next
          },
        },
        {
          type: 'number',
    ...
    },
    {
      ...
      fields: [
        ...
        {
          type: 'number',
          id: 'config-land-share',
          label: 'Grund und Boden',
          hint: 'Nicht abschreibbarer Anteil laut Kaufpreisaufteilung.',
          mode: 'percent',
          min: 0,
          max: 100,
          step: 1,
          get: (value) => value.assumptions.landShare,
          set: (value, next) => {
            value.assumptions.landShare = next
          },
        },
        {
          type: 'number',
          id: 'config-old-building-share',
          label: 'Altbausubstanz',
          hint: 'Normal abschreibbarer Altbauanteil.',
          mode: 'percent',
          min: 0,
          max: 100,
          step: 1,
          get: (value) => value.assumptions.oldBuildingShare,
          set: (value, next) => {
            value.assumptions.oldBuildingShare = next
          },
        },
        {
          type: 'number',
          id: 'config-construction-share',
          label: 'Herstellungskosten',
          hint: 'Normal abschreibbarer Herstellungsanteil.',
          mode: 'percent',
          min: 0,
          max: 100,
          step: 1,
          get: (value) => value.assumptions.constructionShare,
          set: (value, next) => {
            value.assumptions.constructionShare = next
          },
        },
        {
          type: 'number',
          ...
          hint: 'Begünstigter Denkmalanteil am Kaufpreis.',
          mode: 'percent',
          min: 0,
          max: 100,
          step: 1,
        ...
        {
          type: 'number',
          id: 'config-ordinary-afa-rate',
          label: 'Normale Gebäude-AfA',
          hint: 'Jährlicher AfA-Satz für Altbausubstanz und Herstellungskosten.',
          mode: 'percent',
          min: 0,
          max: 10,
          step: 0.1,
          get: (value) => value.assumptions.ordinaryAfaRate,
          set: (value, next) => {
            value.assumptions.ordinaryAfaRate = next
          },
        },
        {
          type: 'number',
...
function buildApartmentConfigSections(): ConfigSection[] {
  return config.apartments.map((apartment) => {
    const fieldIdPrefix = sanitizeForDomId(apartment.id)
    return {
      title: apartment.label,
      copy: apartment.subtitle,
      open: false,
      fields: [
        {
          type: 'number',
          id: `config-${fieldIdPrefix}-size`,
          label: 'Größe',
          hint: 'Fläche in m².',
          mode: 'number',
          min: 10,
          max: 200,
          step: 0.05,
          get: (value) => getConfigApartment(value, apartment.id).size,
          set: (value, next) => {
            getConfigApartment(value, apartment.id).size = next
          },
        },
        {
          type: 'number',
          id: `config-${fieldIdPrefix}-rooms`,
          label: 'Zimmer',
          hint: 'Anzahl der Zimmer laut Preisliste.',
          mode: 'number',
          min: 1,
          max: 10,
          step: 1,
          get: (value) => getConfigApartment(value, apartment.id).rooms,
          set: (value, next) => {
            getConfigApartment(value, apartment.id).rooms = next
          },
        },
        {
          type: 'number',
          id: `config-${fieldIdPrefix}-rent`,
          label: 'Miete pro m²',
          hint: 'Wohnungen mit 18 EUR/m², Teileigentum im EG mit 12 EUR/m².',
          mode: 'currency',
          min: 0,
          max: 100,
          step: 0.5,
          get: (value) => getConfigApartment(value, apartment.id).rentPerSqm,
          set: (value, next) => {
            getConfigApartment(value, apartment.id).rentPerSqm = next
          },
        },
        {
          type: 'number',
          id: `config-${fieldIdPrefix}-accessory-area`,
          label: 'Teileigentum m²',
          hint: 'Zugeordnete Fläche des Teileigentums.',
          mode: 'number',
          min: 0,
          max: 200,
          step: 0.05,
          get: (value) => getConfigApartment(value, apartment.id).accessoryArea,
          set: (value, next) => {
            getConfigApartment(value, apartment.id).accessoryArea = next
          },
        },
        {
          type: 'number',
          id: `config-${fieldIdPrefix}-accessory-rent`,
          label: 'TE-Miete pro m²',
          hint: 'Monatliche Nettokaltmiete des zugeordneten Teileigentums.',
          mode: 'currency',
          min: 0,
          max: 100,
          step: 0.5,
          get: (value) => getConfigApartment(value, apartment.id).accessoryRentPerSqm,
          set: (value, next) => {
            getConfigApartment(value, apartment.id).accessoryRentPerSqm = next
          },
        },
        {
          type: 'number',
          id: `config-${fieldIdPrefix}-purchase-price`,
          label: 'Kaufpreis',
          hint: 'Investitionssumme vor Nebenkosten.',
          mode: 'currency',
          min: 0,
          step: 1000,
          get: (value) => getConfigApartment(value, apartment.id).purchasePrice,
          set: (value, next) => {
            getConfigApartment(value, apartment.id).purchasePrice = next
          },
        },
        {
          type: 'number',
          id: `config-${fieldIdPrefix}-management`,
          label: 'Verwaltung',
          hint: 'Monatliche Verwaltungskosten der Wohnung.',
          mode: 'currency',
          min: 0,
          step: 5,
          get: (value) => getConfigApartment(value, apartment.id).monthlyManagement,
          set: (value, next) => {
            getConfigApartment(value, apartment.id).monthlyManagement = next
          },
        },
        {
          type: 'number',
          id: `config-${fieldIdPrefix}-maintenance`,
          label: 'Rücklage',
          hint: 'Monatliche Instandhaltungskosten der Wohnung.',
          mode: 'currency',
          min: 0,
          step: 5,
          get: (value) => getConfigApartment(value, apartment.id).monthlyMaintenance,
          set: (value, next) => {
            getConfigApartment(value, apartment.id).monthlyMaintenance = next
          },
        },
      ],
    }
...
function buildConfigFromForm(form: HTMLFormElement): CalculationConfig {
  ...
  updateVariableApartmentInConfig(nextConfig, selectedApartmentSize)
  normalizeDerivedConfigValues(nextConfig)
  return validateConfig(nextConfig)
}

function updateVariableApartmentInConfig(targetConfig: CalculationConfig, size: number): void {
  const apartment = targetConfig.apartments.find((entry) => entry.id === selectedApartmentId)
  if (!apartment) {
    return
  }
  const normalizedSize = clamp(size, apartmentSizeBounds.min, apartmentSizeBounds.max)
  apartment.size = Number(normalizedSize.toFixed(1))
  apartment.purchasePrice = Math.round((baseApartmentPrice / baseApartmentSize) * normalizedSize)
  apartment.rooms = normalizedSize < 45 ? 1 : normalizedSize < 85 ? 2 : 3
  apartment.rentPerSqm = fixedRentPerSqm
  apartment.monthlyManagement = fixedMonthlyManagement
  apartment.monthlyMaintenance = Number((apartment.size * monthlyMaintenancePerSqm).toFixed(2))
  ...
  apartment.accessoryRentPerSqm = 0
  apartment.subtitle = buildApartmentSubtitle(apartment)
  targetConfig.assumptions.rentPerSqm = fixedRentPerSqm
  targetConfig.assumptions.monthlyManagementFlat = fixedMonthlyManagement
  targetConfig.assumptions.monthlyMaintenanceFlat = apartment.monthlyMaintenance
  targetConfig.assumptions.monumentShare = fixedMonumentShare
}

function normalizeDerivedConfigValues(nextConfig: CalculationConfig): void {
  nextConfig.apartments.forEach((apartment) => {
    apartment.rooms = Math.max(1, Math.round(apartment.rooms))
    apartment.accessoryArea = Math.max(0, apartment.accessoryArea)
    apartment.accessoryRentPerSqm = Math.max(0, apartment.accessoryRentPerSqm)
    apartment.accessoryLabel = apartment.accessoryArea > 0 ? apartment.accessoryLabel.trim() : ''
    apartment.subtitle = buildApartmentSubtitle(apartment)
  })
  ...
  nextConfig.assumptions.landShare = Math.max(0, nextConfig.assumptions.landShare)
  nextConfig.assumptions.oldBuildingShare = Math.max(0, nextConfig.assumptions.oldBuildingShare)
  nextConfig.assumptions.constructionShare = Math.max(0, nextConfig.assumptions.constructionShare)
  nextConfig.assumptions.monumentShare = Math.max(0, nextConfig.assumptions.monumentShare)
  nextConfig.assumptions.ordinaryAfaRate = Math.max(0, nextConfig.assumptions.ordinaryAfaRate)
...
function validateConfig(candidate: unknown): CalculationConfig {
  ...
  if (!isApartmentId(defaultApartmentIdValue)) {
    throw new Error('defaultApartmentId muss eine gültige apartment id sein.')
  }
  ...
  const apartments = apartmentsCandidate.map((entry) => {
    ...
    if (!isApartmentId(apartmentIdValue)) {
      throw new Error('Jedes apartment braucht eine gültige id.')
    }
    ...
    return {
      ...
      rooms: asNumber(entry.rooms, 'apartment.rooms'),
      size: asNumber(entry.size, 'apartment.size'),
      rentPerSqm: asNumber(entry.rentPerSqm, 'apartment.rentPerSqm'),
      accessoryLabel: asOptionalText(entry.accessoryLabel),
      accessoryArea: asOptionalNumber(entry.accessoryArea, 0, 'apartment.accessoryArea'),
      accessoryRentPerSqm: asOptionalNumber(entry.accessoryRentPerSqm, 0, 'apartment.accessoryRentPerSqm'),
      purchasePrice: asNumber(entry.purchasePrice, 'apartment.purchasePrice'),
      image: asOptionalText(entry.image),
      galleryImages: Array.isArray(entry.galleryImages)
        ? entry.galleryImages.map((imageEntry, index) =>
            asString(imageEntry, `apartment.galleryImages[${index}]`),
          )
        : asOptionalText(entry.image) ? [asOptionalText(entry.image)] : [],
  ...
  const assumptions = {
    ...
    kfwGrantRate: (() => {
      const candidateRate = assumptionsCandidate.kfwGrantRate
      if (candidateRate !== undefined && candidateRate !== null) {
        return asNumber(candidateRate, 'assumptions.kfwGrantRate')
      }
      const grantAmount = asNumber(assumptionsCandidate.kfwGrantAmount, 'assumptions.kfwGrantAmount')
      const loanAmount = asNumber(assumptionsCandidate.kfwLoanAmount, 'assumptions.kfwLoanAmount')
      return loanAmount > 0 ? grantAmount / loanAmount : 0
    })(),
    ...
    landShare: asOptionalNumber(assumptionsCandidate.landShare, 0.12, 'assumptions.landShare'),
    oldBuildingShare: asOptionalNumber(
      assumptionsCandidate.oldBuildingShare,
      0.16,
      'assumptions.oldBuildingShare',
    ),
    constructionShare: asOptionalNumber(
      assumptionsCandidate.constructionShare,
      0.1,
      'assumptions.constructionShare',
    ),
    monumentShare: asNumber(assumptionsCandidate.monumentShare, 'assumptions.monumentShare'),
    ordinaryAfaRate: asOptionalNumber(
      assumptionsCandidate.ordinaryAfaRate,
      0.02,
      'assumptions.ordinaryAfaRate',
    ),
...
function buildApartmentSubtitle(
  apartment: Pick<ApartmentOption, 'rooms' | 'size' | 'accessoryLabel' | 'accessoryArea'>,
): string {
  const residencePart = `${Math.max(1, Math.round(apartment.rooms))}-Zimmer, Wohnung ${formatArea(apartment.size)} m²`
  if (apartment.accessoryArea > 0) {
    const accessoryLabel = apartment.accessoryLabel.trim() || 'Teileigentum'
    return `${residencePart} + ${accessoryLabel} ${formatArea(apartment.accessoryArea)} m²`
  }
  return residencePart
}
...
function buildEmbeddedDefaultPreset(sourceConfig: CalculationConfig): RuntimePreset {
  ...
  return {
    ...
    label: 'Krampnitzcarre 2 Standard',
    version: '2026.05.10-0001',
...
function applyRuntimePreset(preset: RuntimePreset): void {
  ...
  selectedApartmentSize = getApartment(selectedApartmentId).size
  updateVariableApartment(selectedApartmentSize)
...
function appendScenarioParams(
  ...
  params.set('size', String(selectedApartmentSize))
