...
type Assumptions = {
  ...
  kfwZinsbindungJahre: number
  bankZinsbindungJahre: number
  refinanceInterestRate: number
  refinanceRepaymentRate: number
  afaBaseShare: number
  specialAfa7bPerSqm: number
  specialAfa7bYears: number
...
type ProjectionResult = {
  ...
  degressivePhaseMonthlyLiquidity: number
  linearPhaseMonthlyLiquidity: number
  degressivePhaseYears: number
  linearPhaseYears: number
  depreciationSwitchYear: number | null
...
const CONFIG_STORAGE_KEY = 'hafenwerk-runtime-config'
const PREVIEW_PRESET_STORAGE_KEY = 'hafenwerk-preview-preset'
const PREVIEW_PRESET_QUERY_KEY = 'previewPreset'
const DEFAULT_PRESET_ID = 'hafenwerk-default'
...
const MAPS_URL = 'https://www.google.com/maps/search/?api=1&query=An+der+Jutespinnerei+1-13,+04179+Leipzig'
...
const CUSTOMER_SCENARIO_DATA_ROOT = '/Hafenwerk-data/customer-scenarios'
...
const heroSlides: HeroSlide[] = [
  {
    image: '/project/hero-hafenwerk-03.png',
    alt: 'Hafenwerk Expose Seite 3',
    caption: 'Projektansicht',
  },
  {
    image: '/project/hero-hafenwerk-04.png',
    alt: 'Hafenwerk Expose Seite 4',
    caption: 'Projektansicht',
  },
  {
    image: '/project/hero-hafenwerk-05.png',
    alt: 'Hafenwerk Expose Seite 5',
    caption: 'Projektansicht',
  },
  {
    image: '/project/hero-hafenwerk-06.png',
    alt: 'Hafenwerk Expose Seite 6',
    caption: 'Projektansicht',
  },
  {
    image: '/project/hero-hafenwerk-01.png',
    alt: 'Hafenwerk Expose Seite 1',
    caption: 'Hafenwerk Expose',
  },
  {
    image: '/project/hero-hafenwerk-02.png',
    alt: 'Hafenwerk Expose Seite 2',
    caption: 'Grundriss und Ausstattung',
  },
]
...
app.innerHTML = `
              ...
              <div class="config-grid config-grid-compact">
                <label class="config-field">
                  ...
                  <span class="config-field-hint">
                    Wird als Dateiname und URL-Parameter genutzt, z. B. hafenwerk-standard-01.
                  </span>
                  <div class="config-input-wrap">
                    <input
                      ...
                      spellcheck="false"
                      autocomplete="off"
                      placeholder="hafenwerk-standard-01"
                    />
                  </div>
                </label>
                <label class="config-field">
  ...
  <main class="page">
    ...
    <section class="journey-strip" aria-label="So funktioniert der Immobilien-Check">
      <article class="journey-step">
        ...
        <div>
          ...
          <p class="journey-step-copy">Wohnung WE 2-43 auswählen und den Grundriss direkt prüfen.</p>
        </div>
      </article>
      <article class="journey-step">
    ...
    <section class="panel facts-panel">
      <h2>Ein paar schnelle Fakten über Leipzig.</h2>
      <p class="lead">
        Vier starke Standortargumente für den Immobilienmarkt Leipzig im Überblick.
      </p>
      <div class="facts-grid">
        <article class="fact-card">
          <p class="fact-number">633.000+</p>
          <p class="fact-title">Einwohner</p>
          <div class="fact-bar">
            <div class="fact-fill fact-fill-demand" style="width: 84%"></div>
          </div>
          <p class="fact-copy">Eine der wachstumsstärksten Großstädte Deutschlands.</p>
          <p class="fact-copy">Rund +65.000 Einwohner in 10 Jahren.</p>
        </article>

        <article class="fact-card">
          <p class="fact-number">3.784 €</p>
          <p class="fact-title">Medianlohn monatlich</p>
          <div class="compare-row">
            <span>Steigende Einkommen</span>
            <span>+6,6 % Wachstum</span>
          </div>
          <div class="fact-bar">
            <div class="fact-fill fact-fill-single" style="width: 66%"></div>
          </div>
        </article>

        <article class="fact-card">
          <p class="fact-number">17,00–19,00 €/m²</p>
          <p class="fact-title">LEWO „NEUE FÄRBEREI“ (Neubau/Erstbezug)</p>
          <div class="compare-row">
            <span>Immobilienscout-Ausweisung</span>
            <span>Leipzig-Möckern</span>
          </div>
          <div class="dual-bars">
            <div class="dual-bar">
        ...
        <article class="fact-card">
          <p class="fact-number">2–3 %</p>
          <p class="fact-title">Leerstand</p>
          <p class="fact-copy">Sehr niedriger Leerstand mit klarem Nachfrageüberhang.</p>
          <p class="fact-copy">In beliebten Lagen sogar unter 1 %.</p>
        </article>
      </div>
    </section>
  </main>
...
function calculateProjection(
  ...
  const specialAfa7bPerYear = apartment.size * assumptions.specialAfa7bPerSqm
  const kfwZinsbindungEndProjectionYear = assumptions.kfwZinsbindungJahre + 1
  ...
  let kfwRefinanceDebtBase = 0
  let bankRefinanceDebtBase = 0
  let refinanceDebtBase = 0
  let kfwRefinanceTargetDebtService = 0
  let bankRefinanceTargetDebtService = 0
  let cumulativeCashflow20 = 0
  let afaCashflow = 0
  let postAfaCashflow = 0
  let afaYears = 0
  let postAfaYears = 0
  let degressivePhaseCashflow = 0
  let linearPhaseCashflow = 0
  let degressivePhaseYears = 0
  let linearPhaseYears = 0
  let depreciationSwitchYear: number | null = null
  ...
  for (let year = 1; year <= assumptions.years; year += 1) {
    ...
    if (year === kfwZinsbindungEndProjectionYear + 1) {
      kfwRefinanceDebtBase = remainingKfwDebt
      kfwRefinanceTargetDebtService = kfwRefinanceDebtBase * (refinanceInterestRate + refinanceRepaymentRate)
    }
    if (year === bankZinsbindungEndProjectionYear + 1) {
      bankRefinanceDebtBase = remainingBankDebt
      bankRefinanceTargetDebtService = bankRefinanceDebtBase * (refinanceInterestRate + refinanceRepaymentRate)
    }
    refinanceDebtBase = kfwRefinanceDebtBase + bankRefinanceDebtBase

    if (year <= kfwZinsbindungEndProjectionYear) {
      ...
      const kfwRefinanceInterest = remainingKfwDebt * refinanceInterestRate
      const kfwRefinancePrincipal = Math.min(
        Math.max(kfwRefinanceTargetDebtService - kfwRefinanceInterest, 0),
        remainingKfwDebt,
      )
      refinanceInterest += kfwRefinanceInterest
      refinancePrincipal += kfwRefinancePrincipal
      kfwPrincipal = kfwRefinancePrincipal
    }

    if (year <= bankZinsbindungEndProjectionYear) {
      ...
      const bankRefinanceInterest = remainingBankDebt * refinanceInterestRate
      const bankRefinancePrincipal = Math.min(
        Math.max(bankRefinanceTargetDebtService - bankRefinanceInterest, 0),
        remainingBankDebt,
      )
      refinanceInterest += bankRefinanceInterest
      refinancePrincipal += bankRefinancePrincipal
      bankPrincipal = bankRefinancePrincipal
    }

    yearlyDebtService =
      kfwInterest + kfwPrincipal + bankInterest + bankPrincipal + refinanceInterest + refinancePrincipal
    ...
    let usedLinearAfaMethodThisYear = assumptions.depreciationMode === 'linear'
    if (depreciationShare > 0 && remainingDepreciationBase > 0 && usefulLifeYears > 0) {
      if (assumptions.depreciationMode === 'degressive_auto' && !switchedToLinear) {
        ...
        if (linearAmount >= degressiveAmount) {
          ...
          usedLinearAfaMethodThisYear = true
          if (depreciationSwitchYear === null) {
            depreciationSwitchYear = calendarYear
          }
        } else {
          yearlyAfaAmount = degressiveAmount
          usedLinearAfaMethodThisYear = false
        }
      } else {
        const remainingUsefulLife = Math.max(usefulLifeYears - consumedDepreciationYears, 0.0001)
    ...
    const specialAfaShare = getSpecialAfa7bFactorForProjectionYear(year)
    const yearlySpecialAfaAmount =
      assumptions.specialAfa7bPerSqm > 0 && assumptions.specialAfa7bYears > 0
        ? specialAfa7bPerYear * specialAfaShare
        : 0
    const yearlyTotalDepreciation = yearlyAfaAmount + yearlySpecialAfaAmount

    const yearlyTotalInterest = kfwInterest + bankInterest + refinanceInterest
    const vuvEinkuenfte =
      yearlyGrossRent - yearlyDeductibleOperatingCosts - yearlyTotalInterest - yearlyTotalDepreciation
    ...
    if (yearlyAfaAmount > 0) {
      if (assumptions.depreciationMode === 'degressive_auto' && !usedLinearAfaMethodThisYear) {
        degressivePhaseCashflow += yearlyCashflow
        degressivePhaseYears += 1
      } else {
        linearPhaseCashflow += yearlyCashflow
        linearPhaseYears += 1
      }
    }
  ...
  const degressivePhaseMonthlyLiquidity =
    degressivePhaseYears > 0 ? degressivePhaseCashflow / (degressivePhaseYears * 12) : 0
  const linearPhaseMonthlyLiquidity = linearPhaseYears > 0 ? linearPhaseCashflow / (linearPhaseYears * 12) : 0
  const afaPhaseOneMonthlyLiquidity =
    assumptions.depreciationMode === 'degressive_auto'
      ? degressivePhaseMonthlyLiquidity
      : linearPhaseMonthlyLiquidity
  const afaPhaseTwoMonthlyLiquidity =
    assumptions.depreciationMode === 'degressive_auto' ? linearPhaseMonthlyLiquidity : 0
  ...
  return {
    apartment,
    taxTableMode,
    depreciationMode: assumptions.depreciationMode,
    ...
    degressivePhaseMonthlyLiquidity,
    linearPhaseMonthlyLiquidity,
    degressivePhaseYears,
    linearPhaseYears,
    depreciationSwitchYear,
...
function renderBudgetCard(result: ProjectionResult): void {
  ...
  if (result.depreciationMode === 'degressive_auto' && result.depreciationSwitchYear !== null) {
    const mainValue = result.degressivePhaseMonthlyLiquidity
    const linearValue = result.linearPhaseMonthlyLiquidity
    const toneClass = mainValue >= 0 ? 'tone-positive' : 'tone-negative'
    const degressiveStartYear = assumptions.afaStartYear
    const degressiveEndYear = Math.max(result.depreciationSwitchYear - 1, degressiveStartYear)

    el.innerHTML = `
      <p class="budget-label">Mittlerer Nettoeffekt bis zum AfA-Wechsel</p>
      <span class="budget-hero ${toneClass}">${formatSignedCurrency(mainValue)} / Monat</span>
      <p class="budget-footnote">Degressive AfA (${degressiveStartYear}${result.degressivePhaseYears > 1 ? `–${degressiveEndYear}` : ''}). Ab ${result.depreciationSwitchYear} lineare AfA: ca. ${formatSignedCurrency(linearValue)} / Monat.</p>
    `
    return
  }
...
function getSpecialAfa7bFactorForProjectionYear(year: number): number {
  if (assumptions.specialAfa7bYears <= 0) {
    return 0
  }
  const calendarYear = getCalendarYearForProjectionYear(year)
  const startYear = assumptions.rentStartYear
  const endYear = startYear + Math.max(Math.round(assumptions.specialAfa7bYears) - 1, 0)
  if (calendarYear < startYear || calendarYear > endYear) {
    return 0
  }
  return getRentShareForProjectionYear(year)
}
  ...
  return [
    {
      ...
      fields: [
        ...
        {
          type: 'number',
          id: 'config-kfw-zinsbindung-jahre',
          label: 'Zinsbindung KfW',
          hint: 'Dauer der Zinsbindung für das KfW-Darlehen.',
          mode: 'number',
          min: 5,
          max: 20,
          step: 1,
          get: (value) => value.assumptions.kfwZinsbindungJahre,
          set: (value, next) => {
            value.assumptions.kfwZinsbindungJahre = next
          },
        },
        {
          type: 'number',
          id: 'config-bank-zinsbindung-jahre',
          label: 'Zinsbindung Bank',
          hint: 'Dauer der Zinsbindung für das Bankdarlehen.',
          mode: 'number',
          min: 5,
          max: 20,
          step: 1,
          get: (value) => value.assumptions.bankZinsbindungJahre,
          set: (value, next) => {
            value.assumptions.bankZinsbindungJahre = next
          },
        },
        {
          type: 'number',
    ...
    {
      title: 'Wohnung A',
      copy: 'Wohnung WE 2-43 aus dem Exposé.',
      open: false,
      fields: [
        {
          type: 'number',
    ...
    },
    {
      ...
      fields: [
        ...
        {
          type: 'number',
          id: 'config-special-afa-7b',
          label: '7b Sonder-AfA je m²',
          hint: 'Steuerlich wirksamer Zusatzbetrag in € pro m² und Jahr (Laufzeit fest: 4 Jahre).',
          mode: 'currency',
          min: 0,
          step: 10,
          get: (value) => value.assumptions.specialAfa7bPerSqm,
          set: (value, next) => {
            value.assumptions.specialAfa7bPerSqm = next
          },
        },
      ],
    },
...
function normalizeDerivedConfigValues(nextConfig: CalculationConfig): void {
  ...
  nextConfig.assumptions.kfwZinsbindungJahre = Math.max(1, Math.round(nextConfig.assumptions.kfwZinsbindungJahre))
  nextConfig.assumptions.bankZinsbindungJahre = Math.max(1, Math.round(nextConfig.assumptions.bankZinsbindungJahre))
  nextConfig.assumptions.specialAfa7bPerSqm = Math.max(0, nextConfig.assumptions.specialAfa7bPerSqm)
  nextConfig.assumptions.specialAfa7bYears = 4
...
function validateConfig(candidate: unknown): CalculationConfig {
  ...
  const legacyZinsbindungJahre = asNumber(
    assumptionsCandidate.zinsbindungJahre ?? 10,
  ...
  const assumptions = {
    ...
    kfwZinsbindungJahre: asNumber(
      assumptionsCandidate.kfwZinsbindungJahre ?? legacyZinsbindungJahre,
      'assumptions.kfwZinsbindungJahre',
    ),
    bankZinsbindungJahre: asNumber(
      assumptionsCandidate.bankZinsbindungJahre ?? legacyZinsbindungJahre,
      'assumptions.bankZinsbindungJahre',
    ),
    ...
    specialAfa7bPerSqm: asNumber(
      assumptionsCandidate.specialAfa7bPerSqm ?? 0,
      'assumptions.specialAfa7bPerSqm',
    ),
    specialAfa7bYears: asNumber(
      assumptionsCandidate.specialAfa7bYears ?? 0,
      'assumptions.specialAfa7bYears',
    ),
