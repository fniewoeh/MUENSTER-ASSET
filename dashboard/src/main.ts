...
applyConfigButton.addEventListener('click', () => {
  try {
    ...
    const nextPreset = buildCurrentPreset(validConfig)
    applyRuntimePreset(nextPreset)
    try {
      saveStoredConfig(validConfig)
      setConfigStatus('Konfiguration übernommen und lokal gespeichert.')
    } catch {
      setConfigStatus('Konfiguration übernommen. Lokales Speichern war in diesem Browser nicht möglich.')
    }
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Unbekannter Fehler'
